...
private[spark] class ExecutorPodsSnapshotsStoreImpl(
    ...
    volcanoEnabled: Boolean = false,
    executorIdsToJobs: Map[Long, Job] = Map())
  ...
  override def updatePod(updatedPod: Pod): Unit = SNAPSHOT_LOCK.synchronized {
    currentSnapshot = currentSnapshot.withUpdate(updatedPod, volcanoEnabled, executorIdsToJobs)
    addCurrentSnapshotToSubscribers()
  }

  override def replaceSnapshot(newSnapshot: Seq[Pod]): Unit = SNAPSHOT_LOCK.synchronized {
    currentSnapshot = ExecutorPodsSnapshot(newSnapshot, clock.getTimeMillis(), volcanoEnabled, executorIdsToJobs)
    addCurrentSnapshotToSubscribers()
  }
