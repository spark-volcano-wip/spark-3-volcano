...
import org.apache.spark.volcano.submit.VolcanoSparkSubmitOperation.getGracePeriod

import scala.collection.JavaConverters._

private sealed trait VolcanoSubmitOp extends CommandLineLoggingUtils {
...
private class KillApplication extends VolcanoSubmitOp  {
...
private class ListStatus extends VolcanoSubmitOp {
  override def executeOnPod(pName: String, namespace: Option[String], sparkConf: SparkConf)
      (implicit client: KubernetesClient): Unit = {
...
private[spark] class VolcanoSparkSubmitOperation extends SparkSubmitOperation
  ...
  def execute(submissionId: String, sparkConf: SparkConf, op: VolcanoSubmitOp): Unit = {
    val master = KubernetesUtils.parseMasterUrl(sparkConf.get("spark.master"))

    println("Execute is working")
...
private object VolcanoSparkSubmitOperation {
